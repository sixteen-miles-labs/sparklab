...
def test_engine_auto_budget_charges_disk_host_memory_on_integrated_gpu(monkeypatch):
    ...
    monkeypatch.setattr(
        "sparklab.runtime.engine.cache_budget.resolve_moe_cache_auto", fake_resolve
    )
    monkeypatch.setattr(
        torch.cuda,
...
def test_guard_passes_when_size_covers_one_expert_per_layer():
    from sparklab.runtime.engine.engine import _require_offload_cache_size
...
def test_adjust_config_defaults_moe_cache_auto_for_auto_resolved_offload_backend():
    """Bare `sparklab serve <FTW MoE checkpoint>`: no --moe-backend, no --moe-cache-* flags at all.
    ...
    # decide: a bare "auto" consults ~/.cache/sparklab/benchbw.json, so a box that has run
    # `sparklab bench bw` resolves nvfp4 experts to hybrid instead. Assert the family, not the member.
...
def test_page_table_width_covers_whole_trailing_pages():
    ...
    from sparklab.runtime.engine.engine import _page_table_width
