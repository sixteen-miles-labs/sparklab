...
def test_glm_reasoning_parser_honors_disabled_thinking_with_tools():
    ...
    from sparklab.serving.generation import _make_reasoning_parser
...
def test_omitted_max_tokens_defaults_to_hardcoded_32k():
    from sparklab.serving.generation import DEFAULT_MAX_OUTPUT_TOKENS
...
def test_models_route_publishes_the_model_context_length():
    """`sparklab launch` reads this to size each agent's context window."""
    state = FakeState([])
...
# --------------------------------------------------------------- dsv4 reasoning
from sparklab.serving.reasoning_parser import DSML_TOKEN  # noqa: E402
