...
``metadata`` to build the SparkLab ``ModelConfig``.
"""
...
# GGUF ``general.architecture`` -> SparkLab registry key (a GGUF-specific spec that
...
GGUF_ARCH_TO_REGISTRY: dict[str, str] = {
    ...
    "qwen35": "Qwen3_5BonsaiForCausalLM",
}
    ...
    if names:
        ...
        if present is None:
            raise ValueError(
                ...
                "reconvert the checkpoint with the current sparklab.checkpoint.convert"
            )
