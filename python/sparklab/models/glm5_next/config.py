...
The public checkpoint is a multimodal wrapper, but SparkLab serves and certifies
its language tower.  The tower is a 3:1 KDA/NoPE-MLA hybrid and uses four manifold-
constrained Hyper-Connection streams.  SparkLab supports both the publisher's
...
def parse_config(hf_config: Any) -> ModelConfig:
    ...
    cap = os.getenv("SPARKLAB_GLM5_NEXT_MAX_LAYERS")
    if cap:
    ...
    kda_quant = str(
        os.environ.get("_SPARKLAB_CONVERT_GLM5_KDA_QUANT")
        or _get(
            text,
            "sparklab_kda_quant",
            _get(
                hf_config,
                "sparklab_kda_quant",
                _get(text, "freetoken_kda_quant", _get(hf_config, "freetoken_kda_quant", "none")),
            ),
        )
    ).lower()
    if kda_quant not in {"none", "fp8_pertensor"}:
        raise ValueError(f"unsupported GLM-5.3 KDA quantization: {kda_quant!r}")
    dsa_on = bool(dsa_ids) and os.getenv("SPARKLAB_GLM5_NEXT_DSA", "1") != "0"
    args = Glm5NextArgs(
