...
def _rename(raw_name: str) -> str | None:
    """HF key -> SparkLab state-dict key, or None to skip."""
    if raw_name.startswith(("mtp.", "model.visual.", "visual.")):
        return None
    # ModelOpt FP8 KV-cache static scales (full-attention layers only). SparkLab keeps the
...
def setup_offload_expert_banks(
    ...
    demand in ``_expert_gemm``). ``SPARKLAB_FP8_EXPERTS=bf16`` instead dequantizes every expert
    ...
    if eq != "fp8_block":
        from sparklab.moe.expert_banks import _PROVIDERS  # nvfp4 -> _nvfp4_banks, none -> _bf16_banks
    ...
    mode = os.environ.get("SPARKLAB_FP8_EXPERTS", "fp8").strip().lower()
    if mode not in ("fp8", "bf16"):
        raise ValueError(f"SPARKLAB_FP8_EXPERTS must be 'fp8' or 'bf16', got {mode!r}")
...
def _build_fp8_expert_banks(
    ...
    stream)."""
    from sparklab.kernels.triton.fp8_block_linear import FP8
    from sparklab.models.weight import experts_scattered, iter_expert_tensors_parallel
    ...
    if pin and layer_sink is not None and not parallel:
        from sparklab.moe.host_banks import alloc_banks
...
def _setup_bf16_dequant_banks(model_path, model_config, device, dummy: bool, *, layer_sink=None):
    from sparklab.models.weight import dummy_moe_expert_sources
    from sparklab.moe.expert_banks import ExpertBanks
