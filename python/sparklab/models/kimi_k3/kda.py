"""Kimi Delta Attention backed by SparkLab's recurrent FLA kernel."""
...
class KimiDeltaAttention(BaseOP):
    def __init__(self, config, layer_id: int):
        ...
        self.q_proj = kimi_linear(config.attn_quant, self.hidden_size, self.projection_size)
        self.k_proj = kimi_linear(config.attn_quant, self.hidden_size, self.projection_size)
        self.v_proj = kimi_linear(config.attn_quant, self.hidden_size, self.projection_size)
        ...
        self.f_a_proj = kimi_linear(config.attn_quant, self.hidden_size, self.head_dim)
        self.f_b_proj = kimi_linear(config.attn_quant, self.head_dim, self.projection_size)
        self.dt_bias = torch.empty(self.projection_size, dtype=torch.float32)
        self.b_proj = kimi_linear(config.attn_quant, self.hidden_size, self.num_heads)
        if not args.kda_full_rank_gate:
            raise NotImplementedError("Kimi K3 serving currently requires its full-rank KDA gate")
        self.g_proj = kimi_linear(config.attn_quant, self.hidden_size, self.projection_size)
        self.o_norm = _SigmoidGatedRMSNorm(self.head_dim, config.rms_norm_eps)
        self.o_proj = kimi_linear(config.attn_quant, self.projection_size, self.hidden_size)
