...
def vision_load_enabled() -> bool:
    ...
    skip building and loading them unless ``SPARKLAB_LOAD_VISION=1`` is set."""
    return os.getenv("SPARKLAB_LOAD_VISION", "0").strip().lower() in _VISION_TRUE
...
def detect_compressed_tensors_nvfp4(hf_config: Any) -> bool:
    ...
    for g in (groups.values() if isinstance(groups, dict) else []):
        ...
        raise ValueError(
            ...
            f"(group_size={group_size}, strategy={strategy!r}); SparkLab serves "
...
class ModelConfig:
    ...
    # Input/output width of one routed expert. Most architectures use ``hidden_size``;
    # latent-MoE models such as Kimi K3 project into a narrower expert space first.
    # Expert-bank allocators must use this value or they double the Kimi bank geometry.
    ...
    # Weight quantization of the routed MoE experts. "none" keeps the default BF16
    ...
    # weight_scale_inv per block). Dense/shared quantization is declared separately.
    expert_quant: str = "none"
    # Quantization of the always-on shared expert can differ from the routed experts.
    # ``None`` preserves the historical behavior (follow ``expert_quant`` for block-FP8),
    # while an explicit value lets mixed checkpoints such as Qwen3.8 keep routed experts
    # in block-FP8 and the publisher-excluded shared expert in BF16.
    shared_expert_quant: str | None = None
    ...
    def routed_expert_hidden_size(self) -> int:
        """Resolved routed-expert width for model-agnostic bank code."""
        return self.expert_hidden_size or self.hidden_size
