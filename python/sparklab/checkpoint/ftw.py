"""SparkLab Weight (FTW) checkpoint: one O_DIRECT-friendly on-disk format for a whole model.
...
def is_ftw_checkpoint(path: str) -> bool:
    """True if ``path`` is a directory holding a SparkLab Weight (FTW) index."""
...
class FTWDiskExpertSource:
    ...
    def __init__(self, reader: FTWReader, descriptors, staging, cache_banks=None,
        ...
        if cache_policy is None:
            cache_policy = os.getenv("SPARKLAB_DISK_CACHE_POLICY", "lru")
        self.cache_policy = cache_policy.strip().lower()
        if self.cache_policy not in {"lru", "layer_lru"}:
            raise ValueError(
                "SPARKLAB_DISK_CACHE_POLICY must be 'lru' or 'layer_lru', "
        ...
        if read_workers is None:
            read_workers = int(os.getenv(
                "SPARKLAB_DISK_READ_WORKERS", str(_DISK_EXPERT_READ_WORKERS)
            ))
...
def load_ftw_banks(
    ...
    * **Flat region** (pre-existing files, and non-streamable formats): one entry per
      ...
      head-offset slice.
    * **Per-layer** (streamable-format conversion, see :mod:`sparklab.checkpoint.convert`):
    ...
    """
    from sparklab.moe.host_banks import HostBank, PinPipeline, alloc_banks
    from sparklab.utils.progress import byte_bar
