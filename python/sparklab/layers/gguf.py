...
onto SparkLab's ``BaseOP``. SparkLab keeps fused projections (qkv, gate_up) as a
...
from sparklab.models.gguf.dequant import (
    BLOCK_SHAPE,
    GGML_BF16,
    GGML_F16,
    GGML_F32,
...
def fused_mul_mat_gguf(x: torch.Tensor, qweight: torch.Tensor, qweight_type: int) -> torch.Tensor:
    ...
    from sparklab.kernels.gguf import (
        ggml_dequantize,
        ggml_mul_mat_a8,
        ggml_mul_mat_vec_a8,
    )
...
class GGUFEmbedding(BaseOP):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        from sparklab.kernels.gguf import ggml_dequantize
