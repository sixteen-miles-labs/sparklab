...
from sparklab.moe.fused import fused_experts_decode_impl, fused_experts_impl, fused_topk
from sparklab.moe.offload_cache import OffloadMoeCache
from sparklab.utils import div_even
...
# default. Set SPARKLAB_HYBRID_OVERLAP=0 to force the serial path (CPU sync before the
# GPU work) -- a measurement-only escape hatch to A/B the overlap benefit.
_HYBRID_OVERLAP = os.getenv("SPARKLAB_HYBRID_OVERLAP", "1") != "0"
...
class OffloadMoELayer(MoELayer):
    ...
    def _decode_hybrid(
        ...
        # Measurement knob: SPARKLAB_HYBRID_OVERLAP=0 syncs the CPU pool *before* the
    ...
    def _expert_gemm(
        ...
        *,
        ...
        if fmt in ("nvfp4_marlin", "nvfp4_b12x"):
            ...
            from sparklab.moe.nvfp4_backends import b12x_fused_experts, marlin_fused_experts
        ...
        if fmt == "nvfp4":
            # SparkLab's Triton inline-dequant kernels over the native ModelOpt
        ...
        if fmt == "ds_fp4":
            ...
            if is_prefill and n is not None:
                from sparklab.moe.fused_ds_fp4 import routed_experts_fp4_prefill
