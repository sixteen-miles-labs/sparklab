...
import torch
from sparklab.core import Req
from sparklab.runtime.kvcache import BaseCacheHandle, MatchResult, create_prefix_cache
from sparklab.utils import align_down, div_ceil
...
def _swa_eviction_interval() -> int:
    raw = os.environ.get("SPARKLAB_SWA_EVICTION_INTERVAL", "128")
    try:
        return max(1, int(raw))
    except ValueError:
        raise ValueError(f"SPARKLAB_SWA_EVICTION_INTERVAL must be an integer, got {raw!r}")
...
class CacheManager:
    ...
    def match_req(self, req: PendingReq) -> MatchResult:
        ...
        if self.is_swa:
            from sparklab.runtime.kvcache.swa_radix_cache import SWACacheHandle
            m = self.prefix_cache.match_prefix(ids)
            return MatchResult(SWACacheHandle(m.cached_len, m.node, m.kv_indices))
        if self.is_hybrid:
            from sparklab.runtime.kvcache.hybrid_radix_cache import HybridCacheHandle
            m = self.prefix_cache.match_prefix(ids)
            return MatchResult(
