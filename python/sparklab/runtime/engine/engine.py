...
import torch
from sparklab.attention import AttnType, attention_backend_info, create_attention_backend
from sparklab.core import Batch, Context, Req, set_global_ctx
from sparklab.runtime.distributed import destroy_distributed, enable_pynccl_distributed, set_tp_info
from sparklab.layers import set_rope_device
from sparklab.models import create_model, load_weight
...
def _validate_attention_backend_choice(config, override, required: frozenset[AttnType]) -> None:
    ...
    from sparklab.attention import validate_attn_backend
    ...
    for part in backend_parts:
        ...
        if info.requires_flashinfer and not _flashinfer_available():
            raise RuntimeError(
                ...
                "not installed. Install it with `pip install 'sparklab[fi]'` (or "
                "'sparklab[accel]'), or use --attention-backend triton."
            )
        if info.requires_sgl_kernel and not _sgl_flash_attn_available():
            raise RuntimeError(
                ...
                "not installed. Install it with `pip install 'sparklab[sgl]'` (or "
                "'sparklab[accel]'), or use --attention-backend triton."
            )
...
class Engine:
    ...
    def _resolve_auto_moe_cache_size(
        ...
        """
        from sparklab.runtime.engine.cache_budget import expert_bytes_per_slot, resolve_moe_cache_auto
    ...
    def _init_offload_moe_cache(self, config: EngineConfig) -> OffloadMoeCache:
        ...
        if cache_factory is None:
            ...
            if config.moe_storage == "disk":
                ...
                from sparklab.checkpoint.ftw import open_ftw_disk_banks
                ...
                logger.info_rank0(
                    "expert banks: bounded FTW disk source "
                    f"({disk_source.num_staging_buffers}-layer staging + "
                    f"{disk_source.cache_capacity} host-LRU entries)"
                )
            else:
    ...
    def _resolve_hybrid_fetch(self, config: EngineConfig, cache) -> None:
        ...
        achieved bandwidths come from the cached `sparklab bench bw` profile (the same one the
        ...
        if fraction is None:
            ...
            logger.warning_rank0(
                "--moe-hybrid-max-fetch auto: no usable `sparklab bench bw` profile for "
    ...
    def _target_moe_and_expert_bytes(self, moe_cache_size: int | None) -> tuple[int, int]:
        from sparklab.runtime.engine.cache_budget import expert_bytes_per_slot
...
def _adjust_config(config: EngineConfig):
    ...
    if is_moe and config.moe_backend == "auto":
        ...
        # Hardware-adaptive config: a cached `sparklab bench bw` profile can upgrade
        ...
        from sparklab.moe.bench_profile import load_backend_recommendation

        gpu_name = torch.cuda.get_device_name(0) if torch.cuda.is_available() else None
        if load_backend_recommendation(bench_fmt, gpu_name=gpu_name) == "hybrid":
            from sparklab.moe.cpu_executor import compiled_extension_supports
        ...
        if (
            ...
            # backend is already offload-family at *parse* time. A bare `sparklab serve <FTW MoE
