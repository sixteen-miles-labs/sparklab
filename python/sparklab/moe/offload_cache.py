...
# instead of one per bank). Set SPARKLAB_FUSED_COPY=0 to force the legacy per-bank path
# (kept for A/B profiling). Falls back to per-bank automatically if a bank's row bytes or
# base address are not 16-byte aligned.
_FUSED_COPY = os.getenv("SPARKLAB_FUSED_COPY", "1").strip().lower() not in {"0", "false", "no", "off"}
...
class OffloadMoeCache:
    ...
    # "nvfp4" (native ModelOpt rows, SparkLab Triton kernels), "nvfp4_marlin"
    ...
    def set_bank_sources(
        ...
        repackers (see ``_BANK_SCHEMAS`` and :mod:`sparklab.moe.nvfp4_backends`)
        ...
        """
        from sparklab.moe.host_banks import HostResidency
    ...
    def _build_copy_plan(self) -> None:
        ...
        address is not 16-byte aligned, or via SPARKLAB_FUSED_COPY=0.
        """
        self._copy_fused_ok = False
    ...
    def _hit_d2d_usable(self) -> bool:
        ...
        """
        from sparklab.kernels.fast_index_copy import _skip_fast_index_copy_enabled
        ...
        if self._prefill_slot_snapshot is None or self.prefill_copy_stream is None:
            ...
            reason = "SPARKLAB_SKIP_FAST_INDEX_COPY is set (the hit gather would be a no-op)"
        elif not self._copy_fused_ok:
            reason = "the fused copy plan is unavailable (bank alignment or SPARKLAB_FUSED_COPY=0)"
    ...
    def _resolve_batch_memcpy(self) -> bool:
        if self._batch_memcpy is None:
            try:
                from sparklab.kernels.batch_memcpy import load_batch_memcpy
    ...
    def _prefetch_split(self, layer_id: int, buffer_id: int) -> None:
        ...
        from sparklab.kernels.fast_index_copy import fast_index_copy_multi_jit
        from sparklab.moe.offload_kernels import prefill_hit_compact
    ...
    def ensure_experts(
        ...
        from sparklab.moe.offload_kernels import ensure_experts
    ...
    def ensure_experts_hybrid(self, layer_id: int, expert_ids: torch.Tensor) -> None:
        ...
        from sparklab.moe.offload_kernels import ensure_experts_hybrid
    ...
    def materialize_layer(self, layer_id: int) -> None:
        from sparklab.moe.offload_kernels import materialize_layer
    ...
    def copy_missing(self) -> None:
        ...
        from sparklab.kernels import fast_index_copy_jit
...
def iter_offload_moe_layers(model) -> Iterator:
    from sparklab.layers import BaseOP, OffloadMoELayer
