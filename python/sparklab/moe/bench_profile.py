"""Torch-free reader for the ``sparklab bench bw`` hardware profile (``benchbw.json``).
...
def default_profile_path() -> str:
    """New SparkLab bandwidth-profile path (writer destination)."""
...
def _usable_profile(gpu_name: str | None, path: str | None) -> dict | None:
    ...
    ``path`` overrides the profile location. Otherwise ``SPARKLAB_BENCHBW_PATH``
    wins, followed by the standard SparkLab cache location.
    """
    configured = os.environ.get("SPARKLAB_BENCHBW_PATH")
...
def load_backend_recommendation(
    ...
    # Preferred: the per-dtype tuning verdicts (`sparklab bench bw --dtype`), a direct format->backend
    ...
    # Fallback: a per-model profile (`sparklab bench bw --model`). Aggregate the workloads sharing this
