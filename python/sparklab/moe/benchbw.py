"""``sparklab bench bw``: measure host RAM vs PCIe bandwidth and pick the offload-family
...
per-model detail. Results are written to a JSON file (default ``$XDG_CACHE_HOME/sparklab/
benchbw.json``) so the choice is reproducible.

    sparklab bench bw                              # per-dtype tuning bench (default: all formats)
    sparklab bench bw --dtype nvfp4,bf16           # only these formats
    sparklab bench bw --model qwen3.6-moe          # per-model detail instead
    sparklab bench bw --dtype all --model all      # both
"""
...
from sparklab.moe.cpu_executor import physical_core_cpus, resolve_threads_and_affinity
...
# CPU MoE ISA tiers, forced via SPARKLAB_CPU_MOE_ISA (the kernel caps down to hw/build
...
def default_out_path() -> str:
    ...
    from sparklab.moe.bench_profile import default_profile_path
...
def measure_cpu_moe_bw(fmt: str, wl: Workload, iters: int = 64, num_threads: int = 0,
    ...
    subset. ``isas`` (a list) additionally forces each ``SPARKLAB_CPU_MOE_ISA`` tier over
...
def run_benchbw(
    ...
    # while the bench runs -- mirrors sparklab checkpoint's SPARKLAB_CONVERT lines. `done`/`total` count the
    # measured units (ceilings + one per benched format); `label` is what's measuring now.
    prog_on = os.environ.get("SPARKLAB_BENCH_PROGRESS") == "1"
    ...
    def _prog(label: str) -> None:
        if prog_on:
            print(f"SPARKLAB_BENCH {done} {total} {label}", flush=True)
    ...
    exec_log = logging.getLogger("sparklab.moe.cpu_executor")
...
def main(argv: list[str] | None = None, prog: str = "sparklab bench bw") -> int:
