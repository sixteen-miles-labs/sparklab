...
app = FastAPI(title="SparkLab API Server", version=__version__, lifespan=lifespan)
...
async def dispatch_rebuild(
    ...
    *,
    ...
    through ``POST /v1/cache/rebuild`` (``sparklab ctl cache``, the desktop panel, the shell's
...
def _serve_and_run_shell(host: str, port: int) -> None:
    ...
    The shell is an ordinary API client (see ``sparklab.shell``), so shell mode is just
    ``sparklab serve`` plus that client -- one generation path for every caller, none of it
    ...
    the chat as it streams -- ``/v1/requests`` still records every request for ``sparklab ctl``.
    ...
    from sparklab.shell.tui import run_shell
    ...
    thread = threading.Thread(target=server.run, name="sparklab-uvicorn", daemon=True)
    thread.start()
...
def run_api_server(config: ServerArgs, start_backend: Callable[[], "Any"], run_shell: bool) -> None:
    ...
    # Create/validate SPARKLAB_API_LOG_DIR and start the writer thread up front, so a
    ...
    threading.Thread(
        ...
        name="sparklab-backend-supervisor",
        daemon=True,
    ).start()
