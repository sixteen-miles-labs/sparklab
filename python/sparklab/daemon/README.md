...
`sparklab.serving.*` (its `__init__` pulls torch) or `sparklab.utils.*` (its `__init__` pulls
...
isolated `sparklab serve` child. `tests/daemon/test_daemon_import_safety.py` enforces this with an import
sentinel.
...
`--state-dir` (default `~/.sparklab/daemon`, override with `$SPARKLAB_DAEMON_DIR`).
...
`sparklab ctl` targets a running *serve*, not the daemon.
...
Target a non-default daemon with `--url http://host:1900` (or `$SPARKLAB_DAEMON_URL`) and
`--token`/`$SPARKLAB_DAEMON_TOKEN`.
...
| `POST /checkpoint/start\|cancel` | Supervised `sparklab checkpoint` (GPU-exclusive: stops the serve first). |

Set `--token` (or `$SPARKLAB_DAEMON_TOKEN`) to require an `X-SparkLab-Token` header on everything
except `/health`.
