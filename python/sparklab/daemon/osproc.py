...
def is_ft_serve_on_port(pid: int, port: int, *, starttime: int | None = None) -> bool:
    """Verify ``pid`` is (still) an ``sparklab serve`` bound to ``port`` — the re-adoption / liveness
    ...
    looks_like_serve = "serve" in argv and (
        "sparklab.cli" in joined or "sparklab" in joined or os.path.basename(argv[0]) in {"sparklab", "sparklab.exe"}
    )
