"""The supervisor heart: own exactly one ``sparklab serve`` child's lifecycle.

Spawn, kill, and orphan detection all live here, in the daemon's own process, so parent and child
are co-located and ordinary signals suffice. Torch-free: it only ever launches ``sparklab serve`` in a
...
def build_serve_command(
    ...
    """The serve invocation + its log path. ``python -m sparklab.cli serve`` (NOT ``-m
    sparklab``, which is a direct-server entrypoint that ignores subcommand argv) so it uses the
    daemon's own interpreter/venv with no PATH dependency."""
    argv = [python, "-m", "sparklab.cli", "serve", "--model", model, "--port", str(port), *args]
...
class ServeManager:
    ...
    def readopt(self) -> bool:
        ...
        if child is None:
            self._store.clear()
            self._emit(f"stale serve state cleared (pid={state.pid} no longer an sparklab serve)")
            return False
        with self._cond:
            self._child = child
    ...
    def _begin_watch(self, child) -> None:
        if self._tailer_factory is not None:
            ...
            if tailer is not None:
                tailer.start()
        threading.Thread(
            target=self._monitor, args=(child,), name=f"sparklab-daemon-monitor-{child.pid}", daemon=True
        ).start()
    ...
    def _restart_async(self, dead_child) -> None:
        ...
        threading.Thread(target=_run, name="sparklab-daemon-autorestart", daemon=True).start()
