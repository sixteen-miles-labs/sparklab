...
``sparklab.kernels.triton``. ``find_spec`` only checks that the package is
...
def is_triton_kernels_installed() -> bool:
    ...
    ``sparklab.kernels.triton`` reimplements, so its call-site carries its own fallback.
    """
...
def driver_cuda_version() -> int | None:
    ...
    by soname."""
    try:
        from sparklab.kernels.pinned import _load_pinned_extension
