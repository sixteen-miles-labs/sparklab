...
spec name, so the shape lists in :mod:`sparklab.kernels.aot` must cover every
(model, weight-format) pair SparkLab serves. Mirroring flashinfer's ``aot``
...
# prebuilt so the suite also runs under SPARKLAB_DISABLE_JIT=1 release validation.
