...
// (sparklab/kernel/triton/nvfp4_dequant.py): w = E2M1[code] * e4m3(scale) * global.
...
// bytes each). Returns -1 when SPARKLAB_CPU_MOE_PF_BLOCKS is unset: the kernel then
...
static int nvfp4_pf_blocks() {
  static const int v = [] {
    const char* s = getenv("SPARKLAB_CPU_MOE_PF_BLOCKS");
...
static void cumemop_check(int rc, const char* what) {
  ...
  if (rc != 0 && !warned.exchange(true)) {
    std::fprintf(stderr,
                 "[sparklab/cpu_moe] %s failed with CUresult=%d (first occurrence; "
                 ...
                 "set SPARKLAB_CPU_MOE_FLAG_SYNC=0.\n",
                 what, rc);
  }
}
...
// SPARKLAB_CPU_MOE_ISA={scalar,avx2,avx512,avx512bf16} (A/B testing on a machine
// that supports more). SPARKLAB_CPU_MOE_SCALAR=1 forces scalar (legacy alias).
inline IsaTier pick_isa() {
#if CPU_MOE_X86
  if (getenv("SPARKLAB_CPU_MOE_SCALAR")) return ISA_SCALAR;
...
#endif
  if (const char* f = getenv("SPARKLAB_CPU_MOE_ISA")) {
    IsaTier want = best;
...
// Distinct from AVX-512 VNNI. Opt out with SPARKLAB_CPU_MOE_NO_VNNI=1 (A/B the W4A8 path).
inline bool cpu_has_avxvnni() {
#if CPU_MOE_X86
  const char* no = getenv("SPARKLAB_CPU_MOE_NO_VNNI");
...
// Zen4+. 2x the 256-bit AVX-VNNI width. SPARKLAB_CPU_MOE_NO_AVX512VNNI=1 forces the
// 256-bit path (A/B the two W4A8 kernels on the same box); SPARKLAB_CPU_MOE_NO_VNNI=1
...
inline bool cpu_has_avx512vnni() {
  ...
  const char* no = getenv("SPARKLAB_CPU_MOE_NO_AVX512VNNI");
  if (no && no[0] && no[0] != '0') return false;
  const char* no_vnni = getenv("SPARKLAB_CPU_MOE_NO_VNNI");
...
// the ISA tier (gated by cpu_has_avxvnni() / SPARKLAB_CPU_MOE_NO_VNNI), so it wins when
