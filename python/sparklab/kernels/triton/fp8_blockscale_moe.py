...
def fused_experts_decode_fp8_blockscale(
    ...
    if swiglu_limit is None:
        from sparklab.layers import silu_and_mul

        ic2 = torch.empty((M * top_k, inter), device=dev, dtype=dt)
        silu_and_mul(ic1.view(-1, two_i), ic2)
    else:
        from sparklab.kernels.triton.dsv4.fused_moe import fused_swiglu
...
def fused_experts_fp8_blockscale(
    ...
    *, slot_map=None,
    ...
    """
    from sparklab.kernels import moe_sum_reduce_triton
    from sparklab.kernels.triton.fp8_block_linear import per_token_group_quant_fp8
