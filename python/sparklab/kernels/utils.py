...
KERNEL_CACHE_PACKAGE = "sparklab_kernel_cache"
KERNEL_CACHE_DIR_ENV = "SPARKLAB_KERNEL_CACHE_DIR"
DISABLE_KERNEL_CACHE_ENV = "SPARKLAB_DISABLE_KERNEL_CACHE"
...
def _make_name(*args: str) -> str:
    return "sparklab__" + "_".join(str(arg) for arg in args)
...
def _load_prebuilt(name: str) -> Module | None:
    ...
    if cache_dir is None:
        if _env_enabled(DISABLE_JIT_ENV):
            raise RuntimeError(
                "JIT compilation is disabled by SPARKLAB_DISABLE_JIT, "
...
def load_aot(
    *args: str,
    ...
    if cuda_files:
        from sparklab.kernels._toolchain import check_nvcc_matches_torch
