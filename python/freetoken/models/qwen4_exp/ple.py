...
class _CachedRowStore:
    ...
    def _init_row_cache(self) -> None:
        raw = os.getenv("FREETOKEN_QWEN4_NGRAM_CACHE_MB", str(_DEFAULT_ROW_CACHE_MB))
        try:
            cache_mb = int(raw)
        except ValueError as exc:
            raise ValueError(f"invalid FREETOKEN_QWEN4_NGRAM_CACHE_MB={raw!r}") from exc
        if cache_mb < 0:
            raise ValueError("FREETOKEN_QWEN4_NGRAM_CACHE_MB must be non-negative")
