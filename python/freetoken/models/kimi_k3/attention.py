...
class KimiMLAAttention(BaseOP):
    def __init__(self, config: ModelConfig, layer_id: int):
        ...
        self.q_b_proj = LinearReplicated(
            self.q_lora_rank, self.num_heads * self.q_head_dim, has_bias=False
        )
        self.kv_a_proj_with_mqa = LinearReplicated(
            config.hidden_size, self.kv_lora_rank + self.qk_rope_head_dim, has_bias=False
        )
        ...
        self.g_proj = (
            LinearReplicated(config.hidden_size, self.num_heads * self.v_head_dim, has_bias=False)
            if args.mla_output_gate
            else None
        )
        self.o_proj = LinearReplicated(
            self.num_heads * self.v_head_dim, config.hidden_size, has_bias=False
        )
